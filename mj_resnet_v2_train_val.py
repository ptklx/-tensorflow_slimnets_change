...
def train(train_record_file,
    ...
    with slim.arg_scope(resnet_v2.resnet_arg_scope()):
        net, end_points = resnet_v2.resnet_v2_101(inputs=input_images, num_classes=labels_nums, is_training=is_training)



    # Specify the loss function: tf.losses定义的loss函数都会自动添加到loss函数,不需要add_loss()了
    tf.losses.softmax_cross_entropy(onehot_labels=input_labels, logits=net)#添加交叉熵损失loss=1.6
    # slim.losses.add_loss(my_loss)
    loss = tf.losses.get_total_loss(add_regularization_losses=True)#添加正则化损失loss=2.2
    accuracy = tf.reduce_mean(tf.cast(tf.equal(tf.argmax(net, 1), tf.argmax(input_labels, 1)), tf.float32))
...
if __name__ == '__main__':
    ...
    train_record_file='mjdataset/record/train.tfrecords'
    #val_record_file='mjdataset/record_0/train299.tfrecords'
    val_record_file='mjdataset/record/val.tfrecords'
    ...
    snapshot_prefix='mjmodels/resnet_v2/model.ckpt'
